...
const NFT_IDS = [256531, 256492, 256482, 256476, 256331, 256541,256539,256538,256536,256535,256534,256515,256514,256513,256512,256511,256510,256508,256507,256506,256505,256504,256503,256502,256495,256494,256493,256490,256486,256485,256484,256483,256480,256479,256478,256475,256474,256473,256472,256334,256333,256332,256330,256329,256328,256326,256323,256321,256318,256315];
...
function stakeNFT(input) {
  const stakeNFTButton = $('#stakeNFTButton');
    ...
    stakeNFTButton.empty().append(STAKE_BUTTON_CONTENT_ON_TRANSACTION_RUNNING)
		contractNFTStaking.addManyToRegistery(account, nfts).then(tx => {
			const transactionObservable = transactionObservableFactory(tx)
			transactionObservable.subscribe(buttonStateObserver)
...
function claimNFT(input) {
  ...
  if(contractNFTStaking){
    ...
    contractNFTStaking.claimManyFromRegistery(nfts, true, {
            value: 0,
            from: account,
            gasPrice: gasPrice,
        }).then(tx => {
